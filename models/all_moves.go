// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"strconv"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// AllMoves all moves
// swagger:model AllMoves
type AllMoves struct {

	// moves
	// Required: true
	Moves []*Move `json:"moves"`
}

// Validate validates this all moves
func (m *AllMoves) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateMoves(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *AllMoves) validateMoves(formats strfmt.Registry) error {

	if err := validate.Required("moves", "body", m.Moves); err != nil {
		return err
	}

	for i := 0; i < len(m.Moves); i++ {
		if swag.IsZero(m.Moves[i]) { // not required
			continue
		}

		if m.Moves[i] != nil {
			if err := m.Moves[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("moves" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *AllMoves) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AllMoves) UnmarshalBinary(b []byte) error {
	var res AllMoves
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
